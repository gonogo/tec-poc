...
TEC clerks need a global Manage Cases nav link for **Create batch** (journey stub for now).
Browsing existing batches uses the ExUI Case list with case type **Batch** (`TEC_BATCH`), not a
separate batch list URL.
...
| `/tec-create-batch` | Stub page for the Create batch journey (flow TBD) |
...
6. Clicking Create batch opens the stub at `/tec-create-batch`
...
| **Create batch** | `/tec-create-batch` (stub); real journey TBD |
